...
public class ShareSelectCusAdapter extends RecyclerView.Adapter {
    ...
    public ShareSelectCusAdapter(Context mContext, List<MyShareModel.DataBean.CustomerInfoBean> others) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MoreViewHolder(context, mLayoutInflater.inflate(R.layout.recyleview_share_cus2, null));
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        int isAccept = model.getIsAccept();// 1 已选
        if(isAccept == 1) {
            bbnViewHolder.iv_sel.setImageResource(R.mipmap.team_yixuan);
        }else {
            if(model.getSelectType()) {
                bbnViewHolder.iv_sel.setImageResource(R.mipmap.regi_xuanzhong);
            }else {
                bbnViewHolder.iv_sel.setImageResource(R.mipmap.regi_weixuan);
            }
        }

...
import com.mfzn.deepuses.adapter.xiangmu.AddressAdapter;
import com.mfzn.deepuses.adapter.xiangmu.SearchAddressAdapter;
import com.mfzn.deepuses.bass.BaseActivity;
import com.mfzn.deepuses.utils.Constants;
import com.mfzn.deepuses.utils.DataConversionUtils;
...
//    RelativeLayout rlBassCancal;
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        etMapContent.addTextChangedListener(new OnInputChangeListener() {
            @Override
            public void afterTextChanged(Editable s) {
                if (null != s) {
                    if (0 == s.length()) {//没有输入则清空搜索记录
                        mList.clear();
                        ivMapDelete.setVisibility(View.GONE);
                        addressAdapter.setList(mList, "");
                    } else {
                        llMapSearch.setVisibility(View.VISIBLE);
                        ivMapDelete.setVisibility(View.VISIBLE);
                        if (null != location) {
                            doSearchQuery2(s.toString(), location.getCity(), new LatLonPoint(location.getLatitude(), location.getLongitude()));
                        } else {
                            doSearchQuery2(s.toString(), "", null);
                        }
                    }
                }
            }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_map_delete:
                etMapContent.getText().clear();
                break;
        }
    }

    private void initDatas(Bundle savedInstanceState) {
        mvMap.onCreate(savedInstanceState);// 此方法必须重写
        mAMap = mvMap.getMap();
        ...
        mapRecycleview.setLayoutManager(new LinearLayoutManager(this));
        mapRecycleview.setAdapter(mAddressAdapter);

        mList2 = new ArrayList<>();
        addressAdapter = new SearchAddressAdapter(this, mList2);
        mapRecycleview2.setLayoutManager(new LinearLayoutManager(this));
        mapRecycleview2.setAdapter(addressAdapter);

        gson = new Gson();

        mTransAnimator = ObjectAnimator.ofFloat(ivMapWeizhi, "translationY", 0f, -80f, 0f);
        ...
        mAMap.setOnCameraChangeListener(new AMap.OnCameraChangeListener() {
            ...
            public void onCameraChangeFinish(CameraPosition cameraPosition) {
                if (null != location && null != cameraPosition && isSearchData) {
                    ivMapLocation.setImageResource(R.mipmap.location_gps_black);
            ...
            public void onRegeocodeSearched(RegeocodeResult regeocodeResult, int i) {
                if (i == 1000) {
                    if (regeocodeResult != null) {
                        ...
                        mapRecycleview.smoothScrollToPosition(0);
                    }
                }
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
//                            SPUtils.putString(MapLocationActivity.this, Constants.LOCATION_INFO, gson.toJson(location));
                            doWhenLocationSucess();
                        } else {
        ...
        mAddressAdapter.setOnItemClickLisenter(new OnItemClickLisenter() {
            ...
            public void onItemClick(int position) {
                ...
                String wizhi = poiItem.getProvinceName() + "-" + poiItem.getCityName() + "-" + poiItem.getAdName();
//                String adCode = poiItem.getAdCode();
                String address = poiItem.getSnippet() + poiItem.getTitle();

                LatLonPoint latLonPoint = poiItem.getLatLonPoint();
                String latitude = latLonPoint.getLatitude() + "";//纬度
                String longitude = latLonPoint.getLongitude() + "";//经度

                Intent intent = new Intent();
                intent.putExtra(Constants.MAP_LOCATION_WEIZHI, wizhi);
//                intent.putExtra(Constants.MAP_LOCATION_CODE, adCode);
                intent.putExtra(Constants.MAP_LOCATION_JINGDU, longitude);
                intent.putExtra(Constants.MAP_LOCATION_WEIDU, latitude);
                intent.putExtra(Constants.MAP_LOCATION_ADDRESS, address);
...
//                }
    ...
    private void initSearchListener() {
        ...
        mOnItemClickLisenter = new OnItemClickLisenter() {
            @Override
            public void onItemClick(int position) {
                PoiItem poiItem = mList2.get(position);
                String wizhi = poiItem.getProvinceName() + "-" + poiItem.getCityName() + "-" + poiItem.getAdName();
//                String adCode = poiItem.getAdCode();
                String address = poiItem.getSnippet() + poiItem.getTitle();
...
//                intent.putExtra(Constants.MAP_LOCATION_CODE, adCode);
        ...
        addressAdapter.setOnItemClickLisenter(mOnItemClickLisenter);

        mOnPoiSearchListeners = new PoiSearch.OnPoiSearchListener() {
            @Override
            public void onPoiSearched(PoiResult result, int i) {
                if (i == 1000) {
                    if (result != null && result.getQuery() != null) {// 搜索poi的结果
                        if (result.getQuery().equals(mQuery)) {// 是否是同一条
                            ...
                            mList2.addAll(result.getPois());// 取得第一页的poiitem数据，页数从数字0开始
                            if (null != addressAdapter) {
                                addressAdapter.setList(mList2, etMapContent.getText().toString().trim());
                                mapRecycleview2.smoothScrollToPosition(0);
                            }
                        }
                    }
                }
     ...
     * 开始进行poi搜索
     */
    protected void doSearchQuery2(String keyWord, String city, LatLonPoint lpTemp) {
        mQuery = new PoiSearch.Query(keyWord, "", city);//第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
        ...
        mPoiSearch = new PoiSearch(this, mQuery);
        mPoiSearch.setOnPoiSearchListener(mOnPoiSearchListeners);
        if (lpTemp != null) {
            mPoiSearch.setBound(new PoiSearch.SearchBound(lpTemp, 100000, true));//该范围的中心点-----半径，单位：米-----是否按照距离排序
        }
        mPoiSearch.searchPOIAsyn();// 异步搜索
    }

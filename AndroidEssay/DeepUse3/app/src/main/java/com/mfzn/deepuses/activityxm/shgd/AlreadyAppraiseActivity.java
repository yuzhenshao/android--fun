...
public class AlreadyAppraiseActivity extends BaseMvpActivity<AlreadyAppraisePresent> {
    ...
    public int getLayoutId() {
        return R.layout.activity_already_appraise;
    }

    @Override
    public AlreadyAppraisePresent newP() {
        return new AlreadyAppraisePresent();
    }
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        if(shType == 1) {//0全部  1故障保修  2维护升级
            ...
            tvalreTypename.setTextColor(getResources().getColor(R.color.color_62C33A));
        }
        tvalreTypename.setText(dataBean.getShTypeName());
        tvalreAddress.setText(dataBean.getDetailAddress());
        tvalreName.setText(dataBean.getContactName());
        contactPhone = dataBean.getContactPhone();
        tvalrePhone.setText(contactPhone);
                ...
                .setSingleListener(new DialogInterface.OnLeftAndRightClick2Listener<ModifyBmNameDialog>() {
                    ...
                    public void clickRightButton(ModifyBmNameDialog dialog, View view, EditText text) {
                        if(TextUtils.isEmpty(text.getText().toString().trim())) {
                            ToastUtil.showToast(AlreadyAppraiseActivity.this,"请输入删除原因");
                        }else {
